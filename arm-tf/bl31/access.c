...
int init_ccaext_periph_etr_gpt(void){
	...
	* it follows the configuration in CPU GPT
	*/
	datafiller(new_gpt_root+0x110,0x8,0xa0000000+0xa0003);

	//GPU MMIO, 0x2d00_0000 - 0x2d00_ffff, (configured as No access)
...
int init_ccaext_cpu_gpt(void){
	...
	/*Challenge Implementation:
	* To configure 0x8_8000_0000 - 0x8_c000_0000,
	* it follows the configuration in CPU GPT (Here)
	* As for here (GPT is 0xa00a_0000 - 0xa00c_0000), we only protect real GPU table as Root
	*/
...
int init_ccaext_gpu_gpt_mempart(uint64_t startaddr){

	//config stub area (0x8_8000_0000 - 0x8_a000_0000)
	datafiller(startaddr,0x10000,0xffffffffffffffff);
	//config real task area (0x8_a000_0000 - 0x8_b000_0000)
...
void gptconfig_disable_gpummio_protect(void){
	...
	//But we do not need to control the entire mmio, just the mmio for JOB and MMU
...
int ccaext_gpt_enable(void){
	...
	// This function will not be called after system initialization
