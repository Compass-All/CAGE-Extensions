...
void bl31_main(void)
{
	...
	//CCAEXT: Config GPTs
	datafiller(0x8b0000000,0x10000000,0x2);

	init_ccaext_cpu_gpt(); //This process will disable the GPT, then we can arbitrary config
	init_ccaext_periph_usb_gpt();
	init_ccaext_periph_hdlcd0_gpt();
	init_ccaext_periph_hdlcd1_gpt();
	init_ccaext_periph_dma_gpt();
	init_ccaext_periph_pcie_gpt();
	init_ccaext_periph_etr_gpt();
	init_ccaext_gpu_gpt();
	init_bitmap_realtask_page();

	//CCAEXT: Config shadow task mechanism
	init_ccaext_shadowtask();
