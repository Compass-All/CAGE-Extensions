	...
	switch (filter_smc_value){
		...
		case 0x7:{
			...
			*/
			...
			//Since size is small, only one descriptor in it.
        	...
        	if(fpga_datatransfer_stubdatabuffer_size&0xfff)aligned_buffer_size+=0x1000;
        	...
        	if(databuffer_real_addr==0){
            	...
            	//Modify it in CPU-Periph GPT and sublevel GPU GPT
			...
			while(cur_transfer_size!=0){
				if(cur_transfer_size>0x1000){cur_transfer_size-=0x1000;}
				else{cur_transfer_size=0;}
        	...
        	if(not_aligned_new_transfer_size&0xfff)aligned_new_transfer_size+=0x1000;
			uint64_t new_transfer_addr=0x8a8000000; 
			...
			if((fpga_datatransfer_stubdatabuffer_size & 0xfff)==0)transfer_size_offs=0x1000;
			else transfer_size_offs=fpga_datatransfer_stubdatabuffer_size & 0xfff;
			
			//To transfer to xdma1. i.e., 0x5010_0000 (user) and 0x5014_0000 (xdma)
			
			/*
			* CAGE-Extension-Note: We input this address based on PCIe configuration on this prototype, 
			*					   modify this in your environment.
			*/
			uint64_t cur_fpga_addr=SELF_CONFIG_FPGA_DATA_ADDR;
			
			uint64_t cur_transfer_buffer_addr=databuffer_real_addr;
			while(remain_transfer_num>0){
				if(start_transfer_id==0x0){start_transfer_id=0x3f;}
				else{start_transfer_id-=1;}
			...
			mmio_write_32(SELF_CONFIG_XDMA1_CHANNEL_ADDR+fpga_datatransfer_channel,new_transfer_addr_low);
			mmio_write_32(SELF_CONFIG_XDMA1_CHANNEL_ADDR+fpga_datatransfer_channel+0x4,new_transfer_addr_hi);
			mmio_write_32(SELF_CONFIG_XDMA1_CHANNEL_ADDR+fpga_datatransfer_channel+0x8,(total_transfer_num-1));


			//Submit to the correct channel.
			uint64_t expected_channel=(fpga_datatransfer_channel & 0xff00)-0x4000 +SELF_CONFIG_XDMA1_CHANNEL_ADDR+0x4;
			uint64_t expected_channel_status=(fpga_datatransfer_channel & 0xff00)-0x4000 +SELF_CONFIG_XDMA1_CHANNEL_ADDR+0x40;

			mmio_write_32(expected_channel,0x0cf83e19);
			//waiting...
            while(1){
                uint32_t channel_status=mmio_read_32(expected_channel_status);
                if(channel_status&0x1){continue;}
                else break;
            }
		...
		case 0xb:{
			//Help to shut down the FPGA channel. x1 is controlval, x2 is offsetval
			uint64_t fpga_control_addr=x2+SELF_CONFIG_XDMA1_CHANNEL_ADDR;
