				...
				int js)
{
	...
	//CCAEXT: Submit the task here
	//current pid is usually different from kctx pid, and sometimes current pid is 0
	int pos=find_kctx_by_kctx_addr((uint64_t)kctx);
	if (pos != -1 && stop_submit){
			asm volatile(
				"ldr x0,=0xc7000005\n"
				"mov x1,%0\n"
				"mov x2,%1\n"
				...
				"smc #0\n"
				:
				:"r"(jc_head),"r"(js),"r"(kctx->mmu.pgd),"r"(num_update_pte_sections)
				:"x0","x1","x2","x3","x4"
			);
