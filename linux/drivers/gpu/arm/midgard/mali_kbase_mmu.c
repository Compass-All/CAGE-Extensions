		...
		struct kbase_sub_alloc **prealloc_sas)
{
	...
	int kctx_pos=find_kctx_by_pid(kctx->pid);
	if (kctx_pos!=-1) root_pool = &kctx->page_table_pool;
		...
		struct kbase_mmu_table *mmut)
{
	...
	int kctx_pos=find_kctx_by_pid(mmut->kctx->pid);
	if (kctx_pos!=-1) 
...
alloc_free:
	if (kctx_pos!=-1) kbase_mem_pool_free(&kbdev->page_table_pool, p, false);
				    ...
				    unsigned long flags)
{
	...
	int kctx_pos=find_kctx_by_pid(mmut->kctx->pid);
	if(kctx_pos!=-1){
		add_update_pte_section(num_update_pte_sections,(start_vpfn<<12), nr);
		num_update_pte_sections+=1;
	}
	...
	while (remain) {
		...
		do {
			...
			if (kctx_pos!=-1) err = kbase_mem_pool_grow(&kbdev->page_table_pool, cur_level);
			else err = kbase_mem_pool_grow(&kbdev->mem_pool, cur_level);
		...
		if (cur_level == MIDGARD_MMU_LEVEL(2)) {
			...
			for (i = 0; i < count; i++) {
				...
				uint64_t pa_desc=kbdev->mmu_mode->entry_set_ate(target,
						phys[i], flags, cur_level);
				
				if(kctx_pos!=-1){
					add_update_pte_entries(next_available_pte_id,pa_desc);
					next_available_pte_id+=1;
				}
			}
		}
		...
		struct kbase_context *kctx)
{
	...
	while (!mmut->pgd) {
		int err;
		if (kctx_pos!=-1) {
			err = kbase_mem_pool_grow(&kbdev->page_table_pool,MIDGARD_MMU_BOTTOMLEVEL);
		}
		else err = kbase_mem_pool_grow(&kbdev->mem_pool,MIDGARD_MMU_BOTTOMLEVEL);
