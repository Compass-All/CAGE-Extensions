...
#ifndef __LIBXDMA_DEBUG__
inline u32 __read_register(const char *fn, void *iomem)
{
	int retread=ioread32(iomem);
	printk("%s r reg 0x%p, result %x",fn,iomem,retread);
	return retread;
}
#define read_register(mem) __read_register(__func__,mem)
#else
...
static int xdma_engine_stop(struct xdma_engine *engine)
{
	...
	w |= (u32)XDMA_CTRL_STM_MODE_WB;

//	printk("Stopping SG DMA %s engine; writing 0x%08x to 0x%p.\n",engine->name, w, (u32 *)&engine->regs->control);
	if(stop_submit){

		// Since FPGA MMIO is controlled, require the Monitor to shut down it.
		uint64_t controlval=w;
		uint64_t offsetval=((uint64_t)(&engine->regs->control))&(0xffff);
				asm volatile(
			"ldr x0,=0xc700000b\n"
			"mov x1,%0\n"
			"mov x2,%1\n"
			"smc #0\n"
			::"r"(controlval),"r"(offsetval)
			:"x0","x1","x2"
		);
...
{
	//This functions is used not only in h2c but also in c2h.
...
//transfer->desc_virt->dst_addr_lo==0
	if(stop_submit){

		//analyze and submit some info to EL3.
		//We can confirm that the transfer size must be small, so that we can check only once to do it.
		uint64_t desc_phys_addr=cpu_to_le32(PCI_DMA_L(transfer->desc_bus))+(cpu_to_le32(PCI_DMA_H(transfer->desc_bus))<<32);
		uint64_t h2c_or_c2h=0; //0 is h2c, 1 is c2h
		uint64_t channel_offs=((uint64_t)(&engine->sgdma_regs->first_desc_lo) & 0xffff);
		if(channel_offs<0x5000){
			//H2C, 0x4080 -- 
			h2c_or_c2h=0;
		}
		else{
		...
		asm volatile(
			...
			"mov %0,x1\n"
			"mov %1,x2\n"
			:"=r"(firstdesc),"=r"(adjacent)
			:"r"(desc_phys_addr),"r"(h2c_or_c2h):"x0","x1","x2"
		);
		// w = (uint32_t)(firstdesc & 0xffffffff);
	...
	*/
	//printk("%s engine 0x%p now running\n", engine->name, engine);
...
{
//	printk("engine_service_shutdown\n");
	int rv;
...
engine_service_final_transfer(struct xdma_engine *engine,
			      ...
			      u32 *pdesc_completed)
{
//	printk("engine_service_final_transfer\n");
	if (!engine) {
		pr_err("dma engine NULL\n");
		return NULL;
	}
...
static u32 engine_service_wb_monitor(struct xdma_engine *engine,
				     u32 expected_wb)
{
//	printk("engine_service_wb_monitor\n");
...
static int map_single_bar(struct xdma_dev *xdev, struct pci_dev *dev, int idx)
{
	 ...
	 * address space
	 */
//	printk("map_single_bar, BAR%d: %llu bytes to be mapped.\n", idx, (u64)map_len);
	...
	//xdma:map_single_bar: BAR0 at 0x50100000 mapped at 0xffffff8022d00000, length=1)
...
static void transfer_dump(struct xdma_transfer *transfer)
{
	...
	printk("xfer 0x%p, state 0x%x, f 0x%x, dir %d, len %u, last %d.\n",
		transfer, transfer->state, transfer->flags, transfer->dir,
		transfer->len, transfer->last_in_request);

	printk("transfer 0x%p, desc %d, bus 0x%llx, adj %d.\n", transfer,
...
//	printk("channel %d, offset 0x%x, dir %d.\n", channel, offset, dir);
...
//		printk("sw desc %d/%u: 0x%llx, 0x%x, ep 0x%llx.\n", i + req->sw_desc_idx, req->sw_desc_cnt, sdesc->addr, sdesc->len, req->ep_addr);
		...
		//printk("src %x %x, dst %x %x\n",xfer->desc_virt[j].src_addr_hi, xfer->desc_virt[j].src_addr_lo, xfer->desc_virt[j].dst_addr_hi, xfer->desc_virt[j].dst_addr_lo);
	}
...
//	printk("transfer_init\n");
	...
	for (i = 0; i < xfer->desc_num; i++) {
		...
		//printk("set next adj at index %d to %u\n", i, next_adj);
...
//	printk("xdma_xfer_submit\n");
	...
	while (nents) {
		...
		//printk("xfer, %u, ep 0x%llx, done %lu, sg %u/%u.\n", xfer->len,req->ep_addr, done, req->sw_desc_idx, req->sw_desc_cnt);
		//transfer_dump(xfer);
...
ssize_t xdma_xfer_submit_nowait(void *cb_hndl, void *dev_hndl, int channel,
				...
				bool dma_mapped, int timeout_ms)
{
	//printk("xdma_xfer_submit_nowait\n");
	struct xdma_dev *xdev = (struct xdma_dev *)dev_hndl;
	struct xdma_engine *engine;
	struct xdma_io_cb *cb = (struct xdma_io_cb *)cb_hndl;
	int rv = 0, tfer_idx = 0;
...
//		printk("xfer %p, len %u, ep 0x%llx, sg %u/%u. nents = %d\n", xfer, xfer->len, req->ep_addr, req->sw_desc_idx, req->sw_desc_cnt, nents);
		       ...
		       int *h2c_channel_max, int *c2h_channel_max)
{
	...
	//need to address the problem here.
